...
{
    this->index = Point::moveTowards(this->index, enemy->getLocation(), speed);
}

void Ninja::slash(Character * enemy)const
{
    if(isAlive() && (index.distance(enemy->getLocation()) < 1.0))////////////////
    {
        enemy->hit(40);
    }
}
...
string Ninja::print()const 
{
    ...
    if(isAlive())
    {
        result +=  ("N " + getName() + "  hit count is " + to_string(hit_count) + " and the location is " + getLocation().print() + "\n");
    }
    else
    {
